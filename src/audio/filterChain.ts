...
export default class FilterChain {
	...
	this.usePanning = filters.panning && Audio.context.createStereoPanner != null;
		...
		if (!reverb.buffer){
			...
			if (!buffer){
				...
				preLoader.load(["data/reverb/sportcentre.m4a"],PRELOADTYPE.audio,function(){
					...
					reverb.buffer = cachedAssets.audio["data/reverb/sportcentre.m4a"] as AudioBuffer;
				});
			}else{
	...
	panningValue(value: number, time?: number) {
		...
		if (typeof value !== "undefined"){
			...
			if (time){
				...
				this.panner.pan.setValueAtTime(this._panningValue,Audio.context.currentTime);
			}
	...
	setState(name: string, value: boolean) {
        ...
        if (name==="panning") this.usePanning=(!!value) && Audio.context.createStereoPanner != null;
