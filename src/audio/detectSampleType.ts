...
export function detectSampleType(file: BinaryStream, sample: Sample, next: Next): SAMPLETYPE | void {
	...
	let decoder: (file: BinaryStream, sample: Sample, next: Next) => void = readRAWsample;
...
export function decodeFileWithAudioContext(file: BinaryStream, sample: Sample, next: () => void) {
	...
	if (converter.sampleRate !== sample.info?.sampleRate) {
		console.log('Could not initiate desired sampleRate of '+ sample.info?.sampleRate +' instead got '+ converter.sampleRate);
	}
