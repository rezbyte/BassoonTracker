...
export default function getSamplerate(
  file: BinaryStream,
  ext: string,
): MetaData {
  const sbuf8 = new Uint8Array(file.buffer);
  const type =
    String.fromCharCode(sbuf8[0]) +
    String.fromCharCode(sbuf8[1]) +
    String.fromCharCode(sbuf8[2]) +
    String.fromCharCode(sbuf8[3]);
  ...
  if (type === "RIFF") {
    ...
    return {
      ...
      sampleRate:
        (sbuf8[18] << 12) + (sbuf8[19] << 4) + ((sbuf8[20] & 0xf0) >> 4),
    ...
    if (opusHead != -1) {
      ...
      return {
        ...
        sampleRate:
          ...
          (sbuf8[opusHead + 5] << 8) +
          (sbuf8[opusHead + 6] << 16) +
          (sbuf8[opusHead + 7] << 24), // original samplerate
      };
    } else {
      ...
      return {
        ...
        sampleRate:
          ...
          (sbuf8[47] << 56),
        bits:
          sbuf8[48] + (sbuf8[49] << 8) + (sbuf8[50] << 16) + (sbuf8[51] << 24),
      };
    }
  } else if (
    String.fromCharCode(sbuf8[4]) +
      String.fromCharCode(sbuf8[5]) +
      String.fromCharCode(sbuf8[6]) +
      String.fromCharCode(sbuf8[7]) ===
    "ftyp"
  ) {
    ...
    const sampleRate =
      version === 1
        ? (sbuf8[mdhdStart + 16 + 8] << 24) +
          (sbuf8[mdhdStart + 17 + 8] << 16) +
          (sbuf8[mdhdStart + 18 + 8] << 8) +
          sbuf8[mdhdStart + 19 + 8]
        : (sbuf8[mdhdStart + 16] << 24) +
          (sbuf8[mdhdStart + 17] << 16) +
          (sbuf8[mdhdStart + 18] << 8) +
          sbuf8[mdhdStart + 19];
    return {
      type: SAMPLETYPE.MP4,
      sampleRate,
  ...
  function getMP3Header(buf: Uint8Array): MP3Header {
    /* read samplerate from frame: https://de.wikipedia.org/wiki/MP3#Frame-Header
		...
		*/
    for (let i = 500; i < buf.length - 1; i++) {
      // before it's not our header
      if (buf[i] === 0xff && (buf[i + 1] & 0xe0) === 0xe0) {
        // header found
        const MP3_translate_ID = [
          "MPEG Version 2.5",
          "reserved",
          "MPEG Version 2",
          "MPEG Version 1",
        ];
        const MP3_translate_Layer = [
          "reserved",
          "Layer III",
          "Layer II",
          "Layer I",
        ];
        const MP3_translate_numChannels =
          /*["Stereo", "Joint Stereo", "2 Mono", "Mono"]*/ [2, 2, 2, 1];
        ...
        return {
          ...
          sampleRate: !isNaN(MP3_baseRate[srate] / MP3_divisor[ID])
            ? MP3_baseRate[srate] / MP3_divisor[ID]
