...
export function encodeRIFFsample(sampleData: number[], bits: number): BinaryStream {

	let length = sampleData.length;

	const sampleRate = 8000; // how can we know this from raw samples?
    ...
    const buffer = new ArrayBuffer(46 + chunkLength);
	const file = new BinaryStream(buffer,false);
    file.goto(0);
