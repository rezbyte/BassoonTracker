export type WaveFormGenerator = (period: number, progress: number, freq: number, amp: number) => number;

const waveFormFunction = {
    sine: function(period: number, progress: number, freq: number, amp: number): number {
    ...
    saw : function(period: number, progress: number, freq: number, amp: number): number {
        let value = 1 - Math.abs(((progress * freq/7) % 1)); // from 1 to 0
        value = (value * 2) - 1; // from -1 to 1
        value = value * amp * -2;
        return period + value;
    },
    square : function(period: number, progress: number, freq: number, amp: number): number {
        let value = Math.sin(progress * freq) <= 0 ? -1 : 1;
        value = value * amp * 2;
        return period + value;
    },
    sawInverse : function(period: number, progress: number, freq: number, amp: number): number {
