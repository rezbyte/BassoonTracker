...
interface NoteEnvelopes {volume?: GainNode, panning?: StereoPannerNode, scheduled: Scheduled}
...
export default class Instrument {
	...
	volumeEnvelope: Envelope = {type: 0, raw: [], enabled: false, points: [[0,48],[10,64],[20,40],[30,18],[40,28],[50,18]], count:6, loop: false, loopStartPoint: 0, loopEndPoint: 0, sustain: false, sustainPoint: 0};
	panningEnvelope: Envelope  = {type: 0, raw: [], enabled: false, points: [[0,32],[20,40],[40,24],[60,32],[80,32]], count:5, loop: false, loopStartPoint: 0, loopEndPoint: 0, sustain: false,  sustainPoint: 0};
	...
	play(noteIndex: number,notePeriod: number, volume?: number, track?: number, trackEffects?: Effects, time?: number): NoteInfo | null {
		if (Tracker.inFTMode()) {
	...
	noteOn(time: number): NoteEnvelopes {
		let volumeEnvelope;
		let panningEnvelope;
		const scheduled: Scheduled = {volume: undefined, panning: undefined, ticks: undefined, vibrato: undefined, vibratoFunction: undefined};
		...
		if (this.panningEnvelope.enabled && Audio.canUsePanning){
			...
			const scheduledTime = this.processEnvelop(envelope,panningEnvelope,time);
	...
	noteOff(time: number, noteInfo: NoteInfo): 0 | 100 {
		...
		if (Tracker.inFTMode()){
            ...
            if (this.panningEnvelope.enabled && Audio.canUsePanning && noteInfo.panningEnvelope){
	...
	private processEnvelop(envelope: Envelope, audioNode: GainNode | StereoPannerNode, time: number): number | false {
		const tickTime = Tracker.getProperties().tickTime;
	...
	scheduleEnvelopeLoop(audioNode: GainNode | StereoPannerNode, startTime: number, seconds: number, scheduledTime: number = 0): number {
	...
	getAutoVibratoFunction(): WaveFormGenerator {
        switch(this.vibrato.type){
            case 1: return waveFormFunction.square;
            case 2: return waveFormFunction.saw;
            case 3: return waveFormFunction.sawInverse;
        }
