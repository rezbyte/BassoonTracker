...
export default class SampleView extends Panel {
	...
	constructor() { // UI.SampleView()
		...
		this.spinBoxInstrument = new SpinBox({
			...
			min:1,
			font: font,
			onChange : (value) => {Tracker.setCurrentInstrumentIndex(value);}
		});
		...
		const buttonsDisplay: ButtonInfo[] = [
			{label: "Zoom In", width: 62, onClick : () => {this.waveForm.zoom(2);}},
			{label: "Out", width: 38, onClick : () => {this.waveForm.zoom(0.5);}},
			{label: "All", width: 50, onClick : () => {this.waveForm.zoom(1);}},
			{label: "", value: 0, width: 50, type: "number", onSamplePropertyChange: (button,props) => {
					if (typeof props.sampleLength !== "undefined" && button instanceof NumberDisplay) button.setValue(props.sampleLength);
				}},
			{label: "Loop", width: 50, onClick : () => {this.waveForm.zoom(RANGE.loop);}},
			{label: "", value: 0, width: 50, type: "number", onSamplePropertyChange: (button,props) => {
					if (typeof props.loopLength !== "undefined" && button instanceof NumberDisplay) button.setValue(props.loopLength);
					if (typeof props.interal_loopLength !== "undefined" && button instanceof NumberDisplay) button.setValue(props.interal_loopLength);
				}},
			{label: "Range", width: 50, onClick : () => {this.waveForm.zoom(RANGE.range);}},
			{label: "", value: 0, width: 50, type: "number", onSamplePropertyChange: (button,props) => {
					if (typeof props.rangeLength !== "undefined" && button instanceof NumberDisplay) button.setValue(props.rangeLength);
				}}
		];
		...
		const buttonsSelect: ButtonInfo[] = [
			{label: "[", width: 15, onClick : () => {
					this.waveForm.select(RANGE.start);
				}},
			{label: "All", width: 70, onClick : () => {
					this.waveForm.select(RANGE.all);
				}},
			{label: "]", width: 15, onClick : () => {
					this.waveForm.select(RANGE.end);
				}},
			{label: "None" , width: 50, onClick : () => {
					this.waveForm.select(RANGE.none);
				}},
			{label: "Loop", width: 50 , onClick : () => {
					this.waveForm.select(RANGE.loop);
				}},
			{label: "Cut", width: 50, onClick : () => {
					UI.cutSelection();
				}},
			{label: "Copy", width: 50, onClick : () => {
					UI.copySelection();
				}},
			{label: "Paste", onClick : () => {
					UI.pasteSelection();
		...
		this.loopEnabledCheckbox.onToggle = (checked) => {
			const instrument = this.currentInstrumentIndex === null ? null : Tracker.getInstrument(this.currentInstrumentIndex);
		...
		EventBus.on(EVENT.instrumentChange,(value: number) => {
			...
			if (instrument){
				...
				if (instrument.sample){
					...
					this.repeatLengthSpinbox.setValue(instrument.sample.loop.length,true);
					this.spinBoxRelativeNote.setValue(instrument.sample.relativeNote,true);
					this.loopEnabledCheckbox.setState(instrument.sample.loop.enabled,true);
		...
		EventBus.on(EVENT.samplePlay,(context: NoteInfo) => {
			...
			if (context && context.instrumentIndex === this.currentInstrumentIndex){
				const offset = context.effects && context.effects.offset ? context.effects.offset.value : 0;
		...
		EventBus.on(EVENT.trackerModeChanged,(mode: TRACKERMODE) => {
			this.fineTuneSlider.setMax(mode === TRACKERMODE.PROTRACKER ? 7 : 127,true);
			this.fineTuneSlider.setMin(mode === TRACKERMODE.PROTRACKER ? -8 : -128,true);
		...
		EventBus.on(EVENT.samplePropertyChange,(newProps: SamplePropertyChangeData) => {
			const instrument = this.currentInstrumentIndex === null ? null : Tracker.getInstrument(this.currentInstrumentIndex);
			if (instrument){
				if (typeof newProps.loopStart !== "undefined") this.repeatSpinbox.setValue(newProps.loopStart,newProps.internal);
				if (typeof newProps.loopLength !== "undefined") this.repeatLengthSpinbox.setValue(newProps.loopLength,newProps.internal);
			}
		});
	...
	onResize() {
		...
		this.waveForm.setPosition(Layout.col2X,this.inputboxHeight + Layout.defaultMargin + 8);
		this.waveForm.setSize(Layout.col4W,this.height - this.waveForm.top - envelopeHeight - spinButtonHeight - 8);

		this.volumeEnvelope.setPosition(Layout.col2X,this.waveForm.top + this.waveForm.height + Layout.defaultMargin + 30);
		...
		this.sampleDisplayPanel.setPosition(Layout.col2X,this.waveForm.top + this.waveForm.height + Layout.defaultMargin + 30);
		this.sampleSelectPanel.setPosition(Layout.col3X,this.sampleDisplayPanel.top);
		this.sampleEditPanel.setPosition(Layout.col4X,this.sampleDisplayPanel.top);
		this.sampleVolumePanel.setPosition(Layout.col5X,this.sampleDisplayPanel.top);
		...
		this.closeButton.setProperties({
			top: Layout.defaultMargin,
			left: this.instrumentName.left + this.instrumentName.width + Layout.defaultMargin + bitButtonSpace
		});
		...
		this.bit8Button.setProperties({
			...
			width: 20,
			height: 20,
			left: this.instrumentName.left + this.instrumentName.width + Layout.defaultMargin - 2 + bitButtonOffScreen
		});
		this.bit16Button.setProperties({
			top: Layout.defaultMargin,
			width: 20,
			height: 20,
			left: this.instrumentName.left + this.instrumentName.width + Layout.defaultMargin + 18 + bitButtonOffScreen
		});
		...
		const BottomPanelTop = this.waveForm.top + this.waveForm.height + Layout.defaultMargin;
		...
		this.loopEnabledCheckbox.setPosition(this.loopTitleBar.left+2,this.loopTitleBar.top+2);
