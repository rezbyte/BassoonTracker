...
export default class EnvelopePanel extends Panel {
	...
	constructor(type: EnvelopeType) { // UI.EnvelopePanel
		super();
		this.type = type;
		...
		this.buttonAdd.onDown = () => {
			...
			if (this.envelope.points.length > this.envelope.count){
				const prevPoint = this.envelope.points[this.envelope.count-1] || [0,0];
		...
		this.background = new Scale9Panel(0,0,this.panel.width,this.panel.height,Assets.panelMainScale9);
	...
	private checkMax() {
		if (this.envelope?.count){
            if (this.envelope.sustainPoint >= this.envelope.count) this.sustainSpinbox.setValue(this.envelope.count-1);
            if (this.envelope.loopStartPoint >= this.envelope.count) this.loopFromSpinbox.setValue(this.envelope.count-1);
            if (this.envelope.loopEndPoint >= this.envelope.count) this.loopToSpinbox.setValue(this.envelope.count-1);
		}
	};
}