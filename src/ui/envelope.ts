        ...
        for (let i = 0, max = this.currentEnvelope.count; i < max; i++) {
          ...
          if (Math.abs(x - point[0]) < 6 && Math.abs(y - point[1]) < 6) {
            ...
            if (i !== 0) {
              ...
              if (i < this.currentEnvelope.count - 1)
                this.activePoint.maxX = this.currentEnvelope.points[i + 1][0];
            }
            break;
          }
        }
  ...
  onDrag(touchData: Drag) {
    if (this.isDragging) {
      if (this.activePoint === null) {
        console.error(
          "Envelope.onDrag expected activePoint to be processed by onHover()!",
        );
        return;
      }
      if (this.dragPoint === null) {
        console.error(
          "Envelope.onDrag expected dragPoint to be processed by onDragStart()!",
        );
        return;
      }
  ...
  render() {
    if (this.needsRendering) {
      ...
      if (this.currentEnvelope && this.currentEnvelope.count) {
        ...
        this.ctx.strokeStyle = this.currentEnvelope.enabled
          ? "rgba(120, 255, 50, 0.5)"
          : "rgba(120, 120, 180, 0.5)";
        ...
        if (this.currentEnvelope.enabled) {
          ...
          if (this.currentEnvelope.sustain) {
            const sustainPoint =
              this.currentEnvelope.points[
                this.currentEnvelope.sustainPoint || 0
              ];
            if (sustainPoint) {
          ...
          if (this.currentEnvelope.loop) {
            ...
            const loopStartPoint =
              this.currentEnvelope.points[
                this.currentEnvelope.loopStartPoint || 0
              ];
            const loopEndPoint =
              this.currentEnvelope.points[
                this.currentEnvelope.loopEndPoint || 0
              ];
