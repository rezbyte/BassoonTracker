...
export default class MainPanel extends Panel {
    ...
    constructor() { // Formerly UI.MainPanel
        ...
        EventBus.on(EVENT.toggleView,(view: VIEW) => {
            if (view === VIEW.piano){
                ...
                if (this.pianoPanel.isVisible()){
                    ...
                    this.pianoPanel.setPosition(Layout.mainLeft,this.height-this.pianoPanel.height);
        ...
        EventBus.on(EVENT.showContextMenu, (properties: ShowEventHandlerProperties) => {
    ...
    createContextMenu(properties: { name: string | number, x:number, y: number, items: MenuItem[] }) {
