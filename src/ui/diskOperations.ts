...
export default class DiskOperations extends Panel {
	...
	private onLoadChildren: (item: ListBoxItem, data: ListBoxItem[] | Samples | undefined) => void;
	private itemHandler: typeof ModulesPl | typeof ModArchive | typeof Dropbox | null  = null;
	...
	constructor() { // UI.DiskOperations
		...
		EventBus.on(EVENT.diskOperationTargetChange, (target: RadioGroupItem | undefined) => {
			...
			if (typeof target === "undefined") targetString = this.targetPanel.getTarget();
			...
			if (action === "save"){
				...
				const labelText =  this.currentView === "samples" ? "Export Sample" : "Export Module";
		...
		EventBus.on(EVENT.instrumentChange, (value: number) => {
			if (this.isVisible() && this.currentView == "samples") this.label.setLabel("Load Sample to slot " + Tracker.getCurrentInstrumentIndex());
		});
	}
	...
	setView(subView: VIEW) {
        ...
        this.refreshList(this.currentsSubView === VIEW.sampleEditor ? "samples" : "");
	...
	refreshList(type?: string) {
		...
		const addListatLevel = (data: ListBoxItem[], level: number) => {
			data.forEach((item) => {
                let icon;
                if (item.icon && typeof item.icon === "string") icon = Y.getImage(item.icon);
				if (!icon) icon = this.currentView === "modules" ? Y.getImage("module") : Y.getImage("sample");
				if (item.children) icon = Y.getImage("disk");
				items.push({label: item.title ?? item.label, data: 1, level: level, index: index, icon: icon, info: item.info}); // data: item,
		...
		switch (this.currentView){
			case "modules":
				...
				if (this.modules.length){
					...
					FetchService.json(Host.getBaseUrl() + "data/modules.json",(data?: {modules: ListBoxItem[]}) => {
			...
			case "modarchive":
				...
				this.onLoadChildren = (item,data) => {
					if (data !== undefined && !DiskOperations.isSamples(data)){
						...
						item.children = [{title:"error loading data", label:"error loading data",  index: 0}];
				...
				if (this.modArchive.length){
					...
					FetchService.json<{modarchive: ListBoxItem[]}>(Host.getBaseUrl() + "data/modarchive.json",(data) => {
			...
			case "modulespl":
				...
				if (this.modulesPl.length){
					...
					FetchService.json(Host.getBaseUrl() + "data/modulespl.json",(data?: {modulespl: ListBoxItem[]}) => {
					...
					if (item && item.data){
						...
						if (item.children){
							...
							if (item.url === undefined) {
								console.error(`No URL available for item: ${fileName} to load children from!`);
								return;
							}
							...
							Dropbox.getFile(item.url,(blob) => {
								...
								reader.onload = () => {
									Tracker.processFile(reader.result as ArrayBuffer,fileName,(isMod) => {
										UI.setStatus("Ready");
									});
								};
			...
			case "samples":
				this.itemHandler = null;
				this.label.setLabel("Load Sample to slot " + Tracker.getCurrentInstrumentIndex());
				this.listbox.onClick = (e) => {
					let item = this.listbox.getItemAtPosition(e.x, e.y);
					if (item && item.data){
						const index = item.index;
				...
				this.onLoadChildren = (item: ListBoxItem, data: ListBoxItem[] | Samples | undefined) => {
				...
				if (this.samples.length){
					...
					FetchService.json(Host.getBaseUrl() + "data/samples.json",(data?: Samples) => {
	...
	playRandomSong(format?: string) {
		...
		if (useModArchiveAPI){
			...
			UI.setInfo("");
			FetchService.json("https://www.stef.be/bassoontracker/api/random" + (format || ""),(data?: {modarchive: {module: ListBoxItem}}) => {
	...
	private toggleDirectory(item: ListBoxItem, index: number) {
		...
		if (item.isExpanded){
			...
			if (item.children?.length){
				...
				if (item.url === undefined) {
					console.error(`No URL available for directory: ${item.label} to load children from!`);
					return;
				}
	...
	private static isSamples(data: ListBoxItem[] | Samples | undefined): data is Samples {
