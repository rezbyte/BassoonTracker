...
type AssetName =
  ...
  | "buttonDarkGreen"
  | "buttonKey";
type AssetMap<T extends AssetName> = T extends "button20_20"
  ? Button
  : T extends "button30_30"
    ? Scale9Panel
    : T extends "buttonLight"
      ? Button
      : T extends "buttonDark"
        ? Button
        : T extends "buttonDarkBlue"
          ? Button
          : T extends "buttonDarkRed"
            ? Button
            : T extends "buttonDarkGreen"
              ? Button
              : T extends "buttonKey"
                ? Button
                : never;
...
class Assets {
  ...
  preLoad(next: () => void) {
    ...
    FetchService.json(
      assetUrl("skin/spritemap_v4.json"),
      (data: Sprite[] | undefined) => {
        spriteMap = data;
        createSprites();
      },
    );
  ...
  init() {
    ...
    this.buttonDarkGreenActiveScale9.img = Y.getImage(
      "button_inlay_green_active",
    );
    this.buttonDarkRedActiveScale9.img = Y.getImage("button_inlay_red_active");
    this.buttonDarkBlueActiveScale9.img = Y.getImage(
      "button_inlay_blue_active",
    );
    this.buttonDarkYellowActiveScale9.img = Y.getImage(
      "button_inlay_yellow_active",
    );
