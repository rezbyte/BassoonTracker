...
export default class WaveForm extends Element {
	...
	constructor() { // UI.WaveForm 
		...
		this.scrollBar.onDrag=(touchData) => {
			if (this.scrollBar.startLeft === undefined) {
				console.error("WaveForm.onDrag() expected startLeft to be set in WaveForm.onDragStart()!")
				return;
			}
	...
	onDragStart(touchData: Touch) {
		...
		const pixelValue = (this.currentInstrument.sample.length/this.width)/this.zoomAmount;
		this.rangeStart = this.rangeEnd = Math.round(this.zoomStart + (dragRangeStart * pixelValue));
		this.rangeLength = 0;
	...
	onDrag(touchData: Drag){
		...
		if (this.dragMarker && (this.dragMarker === MARKERTYPE.loopStart || this.dragMarker === MARKERTYPE.loopEnd)){
			this.activeDragMarker = this.dragMarker;
			let value = this.dragMarkerStart + Math.round(pixelValue*touchData.deltaX);
			...
			if (this.dragMarker === MARKERTYPE.loopStart){
				...
				if ((newProps.loopStart + this.currentInstrument.sample.loop.length)>this.sampleLength){
					newProps.loopLength = this.sampleLength - newProps.loopStart;
				}
			}else{
				value = Math.max(value,2);
				value = Math.min(value,this.sampleLength-this.currentInstrument.sample.loop.start);
		...
		if (this.dragMarker && (this.dragMarker === MARKERTYPE.rangeStart || this.dragMarker === MARKERTYPE.rangeEnd)){
			this.activeDragMarker = this.dragMarker;
			let value = this.dragMarkerStart + Math.round(pixelValue*touchData.deltaX);
	...
	playSection(section: RANGE.range | RANGE.loop) {
		...
		if (section === RANGE.loop) {
			...
			Input.handleNoteOn(Input.getPrevIndex(),undefined,this.currentInstrument.sample.loop.start);
		}
	...
	zoom(amount: RANGE.range | RANGE.loop | RANGE.all | number) {
		...
		if (amount === RANGE.range){
			if (this.rangeLength){
				...
				this.scrollBar.setPosition(Math.floor((this.zoomStart/(this.sampleLength-this.zoomLength))*sMax),this.scrollBar.top);
				handled = true;
			}else{
		...
		if (this.hasHorizontalScrollBar){
			if (this.zoomEnd>this.sampleLength){
				...
				this.scrollBar.setPosition(this.width - this.scrollBar.width - 1,this.scrollBar.top);
			}
		}
		this.waveformDisplay.refresh();
		this.refresh();
	...
	select(range: RANGE, start?: number, length?: number): void {
		if (this.currentSampleData === null) {
			console.error("Cannot select a range in the loaded sample without a sample loaded!")
			return;
		}
		switch (range) {
			case RANGE.all:
			...
			case RANGE.none:
				...
				break;
			case RANGE.loop:
				if (this.currentInstrument && this.currentInstrument.sample.loop.length>2){
	...
	render(internal?: boolean) {
		...
		if (this.needsRendering) {
			...
			if (this.waveformDisplay.needsRendering){
				...
				this.waveformDisplay.ctx.drawImage(this.background.render(true),0,0,this.width,this.height);

				if (this.currentSampleData && this.currentSampleData.length && this.width){
					...
					for (let i = 0; i<this.width; i++){
						...
						const peak = this.currentSampleData[this.zoomStart + index] * -maxHeight;
			...
			if (this.isPlaying && this.startPlayTime !== null && this.sampleLength && this.sampleRate && this.currentInstrument){
				const now = new Date().getTime();
				const delta = now - this.startPlayTime;
				let index = this.playingOffset + (this.sampleRate * delta)/1000;

				if (this.currentInstrument.sample.loop.enabled && index>this.currentInstrument.sample.loop.start){
					index = this.currentInstrument.sample.loop.start + ((index-this.currentInstrument.sample.loop.start)%this.currentInstrument.sample.loop.length);
					//isPlaying=false;
			...
			if (this.currentInstrument && (this.currentInstrument.sample.loop.length>2 || this.currentInstrument.sample.loop.enabled)){

				const color = this.currentInstrument.sample.loop.enabled ? "rgb(241, 220, 71)" : "rgba(150, 150, 150,0.7)";

				this.ctx.fillStyle = color;
				if (this.activeDragMarker === MARKERTYPE.loopStart) this.ctx.fillStyle = "white";
				...
				this.ctx.fillStyle = color;
				if (this.activeDragMarker === MARKERTYPE.loopEnd) this.ctx.fillStyle = "white";
			...
			if (this.rangeEnd>=0){
				...
				this.ctx.fillStyle = color;
				if (this.activeDragMarker === MARKERTYPE.rangeEnd) this.ctx.fillStyle = "white";
			...
			if (this.rangeStart>=0){
				if (this.rangeStart<this.zoomStart){
					...
					const color = "rgb(241, 131, 71)";
					this.ctx.fillStyle = color;
					if (this.activeDragMarker === MARKERTYPE.rangeStart) this.ctx.fillStyle = "white";
			...
			if (rangeLineX1 !== rangeLineX2){
				...
				this.ctx.fillRect(rangeLineX1,0,rangeLineX2-rangeLineX1,this.height);
			}
	...
	private getLoopMarkerPos(type: MARKERTYPE): number {
		...
		if (type === MARKERTYPE.loopStart){
			...
			const lineX = Math.floor(((loopStart-this.zoomStart)/this.zoomLength) * this.width);
		...
		const lineX = Math.floor(((point-this.zoomStart)/this.zoomLength) * this.width);
		return(Math.min(lineX,this.width-(this.zoomEnd>this.sampleLength-6?6:0)));
	}
	...
	private getRangeMarkerPos(type: MARKERTYPE): number {
		if (type === MARKERTYPE.rangeStart){
			...
			const lineX = Math.floor(((this.rangeStart-this.zoomStart)/this.zoomLength) * this.width);
			return Math.max(this.zoomStart>5?0:5,lineX);
		}
	...
	private splitRange(useEmptyRange: boolean = false): SplitSample {
		...
		if (this.rangeLength){
			const tail = this.currentSampleData.slice(this.rangeStart + this.rangeLength);
			const range = this.currentSampleData.slice(this.rangeStart,this.rangeStart + this.rangeLength);
			...
			if (useEmptyRange){
				...
				const tail: [] = [];
				const range = this.currentSampleData.slice(0,this.currentSampleData.length);
				const head: [] = [];
				return {tail, range, head};
			}
		}
	...
	private checkLoop(){
		...
		if (ls!==this.currentInstrument.sample.loop.start || ll!==this.currentInstrument.sample.loop.length){
	...
	adjustVolume(amount: "max" | "fadein" | "fadeout" | number) {
		...
		const editAction = StateManager.createSampleUndo(SELECTION.REPLACE,this.rangeStart,this.rangeLength);
	...
	resample(direction: "up" | string) {
		...
		if (direction === "up"){
			...
			this.currentInstrument.sample.loop.start = Math.floor(this.currentInstrument.sample.loop.start*2);
			this.currentInstrument.sample.loop.length = Math.floor(this.currentInstrument.sample.loop.length*2);
		...
		if (!Tracker.inFTMode()){
			this.currentInstrument.sample.loop.start = this.currentInstrument.sample.loop.start - this.currentInstrument.sample.loop.start%2;
			this.currentInstrument.sample.loop.length = this.currentInstrument.sample.loop.length - this.currentInstrument.sample.loop.length%2;
		}
		data.range = newRange;
	...
	processSelection(state: SELECTION): false | undefined {
		...
		switch (state) {
			...
			case SELECTION.CUT:
				if (this.rangeLength>0){
					...
					if (state === SELECTION.CUT){
						...
						const editAction = StateManager.createSampleUndo(SELECTION.CUT,this.rangeStart,this.rangeLength);
						editAction.data = data.range.slice(0);
			...
			case SELECTION.PASTE:
				...
				const editAction = StateManager.createSampleUndo(SELECTION.PASTE,this.rangeStart,this.rangeCache.length);
