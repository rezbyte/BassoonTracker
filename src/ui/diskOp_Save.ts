...
export default class DiskOperationSave extends Panel {
	...
	constructor() { // UI.DiskOperationSave
		super();
		this.fileName = "";
		...
		this.background = new Scale9Panel(0,0,20,20,Assets.panelDarkInsetScale9);
		...
		this.selectionType.onChange = (selectedIndex) => {
			...
			this.saveAsFileType = item && item.fileType ? item.fileType : FILETYPE.module;
			this.saveAsFileFormat = item && item.fileFormat ? item.fileFormat : MODULETYPE.mod;
			this.setFileName();
		};
		...
		this.saveButton.onClick = () => {
			if (this.mainFileType == FILETYPE.module){
				...
				if (this.saveAsFileType == FILETYPE.sample){
					...
					BassoonProvider.renderFile(this.fileName,this.saveAsFileFormat === SAMPLETYPE.MP3);
				}
			}
			if (this.mainFileType == FILETYPE.sample){
				...
				if (sample){
					...
					if (this.saveAsFileFormat === SAMPLETYPE.RAW_8BIT){
						...
						file = encodeRIFFsample(sample.data,this.saveAsFileFormat === SAMPLETYPE.RIFF_16BIT ? 16 : 8);
					}
		...
		EventBus.on(EVENT.diskOperationTargetChange,(item?: RadioGroupItem) => {
			...
			if (item && item.fileType !== undefined){
				...
				if (this.mainFileType == FILETYPE.sample) {
					this.fileName = Tracker.getCurrentInstrument().name.replace(/ /g, '-').replace(/\W/g, '');
				}
				if (this.mainFileType == FILETYPE.module) {
		...
		EventBus.on(EVENT.instrumentChange, (value: number) => {
			if (this.isVisible() && this.mainFileType == FILETYPE.sample) {
				this.fileName = Tracker.getCurrentInstrument().name.replace(/ /g, '-').replace(/\W/g, '') || "Sample-" + Tracker.getCurrentInstrumentIndex();
				this.setFileName();
			}
		});
