...
export default class AppControlPanel extends AppPanelContainer {
	...
	constructor() { // UI.app_controlPanel
		super(40);
		...
		this.buttonFileOperations = new CheckboxButton({
			label: "File",
			onDown: () => {
				const view = this.buttonFileOperations.isActive ? VIEW.fileOperations : VIEW.topMain;
		...
		this.buttonSampleEdit = new CheckboxButton({
			label: "Sample Edit",
			onDown: () => {
				const view = this.buttonSampleEdit.isActive ? VIEW.sampleEditor : VIEW.bottomMain;
				EventBus.trigger(EVENT.showView, view);
			}
		});
		...
		this.trackButtons = [];
		this.trackView.forEach((count) => {this.trackButtons.push(new Button());});
		...
		this.trackCountSpinbox.setProperties({
			...
			onChange : (value) => {Tracker.setTrackCount(value)}
		});
    ...
    renderInternal(){
		...
		this.ctx.drawImage(lineVer,Layout.controlPanelButtonsLeft-2,0,2,this.height-1);
		this.ctx.drawImage(lineVer,Layout.modeButtonsLeft-2,0,2,this.height-1);
