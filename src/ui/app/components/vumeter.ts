...
export default class VUMeter extends Panel {
	...
	private buildVu() {
		...
		this.baseActiveCtx.clearRect(0,0,this.baseActive.width,this.baseActive.height);
		...
		for (let i = 0; i< dots; i++){
			...
			this.baseCtx.drawImage(img,i*(this.dotWidth+this.margin),0,this.dotWidth,this.vuHeight);
			this.baseActiveCtx.drawImage(imgActive,i*(this.dotWidth+this.margin),0,this.dotWidth,this.vuHeight);
		}
	...
	render(internal?: boolean) {
		...
		const wLeft = this.dataArray && this.analyserLeft ? this.getW(this.analyserLeft, this.dataArray) : null;
		const wRight = this.dataArray && this.analyserRight ? this.getW(this.analyserRight, this.dataArray) : null;

		if (wLeft) this.ctx.drawImage(this.baseActive,0,0,wLeft,this.vuHeight,0,0,wLeft,this.vuHeight);
		if (wRight) this.ctx.drawImage(this.baseActive,0,0,wRight,this.vuHeight,0,this.vuHeight + this.middleMargin,wRight,this.vuHeight);
