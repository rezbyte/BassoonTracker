...
export default class AppSongPatternList extends Panel {
    ...
    constructor(height?: number) { // UI.app_songPatternList
        ...
        EventBus.on(EVENT.patternTableChange,(value?: number) => {
            const song = Tracker.getSong();
            if (song == null) {
                console.error("AppSongPatternList recieved a patternTableChange event without a song loaded!");
                return;
            }
    ...
    onResize() {
        ...
        this.songlistbox.setProperties({
            ...
            onChange: () =>{
                Tracker.setCurrentSongPosition(this.songlistbox.getSelectedIndex(),true);
            }
        });
    ...
    setPatternTable(song: Song) {
        ...
        for (let i = 0, len = song.length; i<len; i++){
            ...
            items.push({label: this.padd2(i+1) + ":" + this.padd2(value), data:value, index: i});
        }
