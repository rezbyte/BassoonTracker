...
export default class AppMenu extends AppPanelContainer {
  ...
  constructor(container: Element) {
    ...
    menu.setItems([
      ...
      },
      {
        ...
        subItems: [
          {
            label: function () {
              return StateManager.getUndoLabel();
            },
          ...
          {
            label: function () {
              return StateManager.getRedoLabel();
            },
    ...
    if (cutOffVolume == null) {
      console.error(
        "AppMenu needs the cut-off volume but it is not set! Check whether Audio has been initalised correctly.",
      );
    } else {
  ...
  onPanelResize() {
    ...
    const vuLeft =
      Layout.marginLeft + menuWidth + Layout.defaultMargin + Layout.mainLeft;

    this.left = Layout.mainLeft;

    if (menuWidth)
      this.menuBackground.setDimensions({
