...
export default class AppMainPanel extends AppPanelContainer {
  ...
  constructor() {
    ...
    this.listbox.setItems([{ label: "loading ...", data: 1, index: 0 }]);
    ...
    this.listbox.onClick = (touch) => {
      ...
      if (item) {
        if (item.data === undefined) {
          console.error(
            "AppMainPanel instrument listbox received a click on a valid item with no data!",
          );
          return;
        }
    ...
    this.spinBoxSongRepeat = new SpinBox({
      ...
      onChange: (value) => {
        ...
        if (song == null) {
          console.error(
            "Song repeat spinbox tried to change the song restart position without a song loaded!",
          );
          return;
        }
  ...
  private steffestVersion() {
    ...
    if (imgCtx == null) {
      console.error(
        "Failed to get a canvas context to render the version image!",
      );
      return;
    }
  ...
  onPanelResize() {
    ...
    if (Layout.prefered === "col3") {
      ...
      if (this.radioGroup == null) {
        console.error(
          "AppMainPanel.initSmallScreenUI() did not initialize the radio group!",
        );
        return;
      }
  ...
  private initSmallScreenUI() {
    ...
    this.radioGroup.setItems([
      ...
      {
        label: "Song data",
        labels: [{ width: 30, label: "song" }],
        active: false,
      },
      {
        label: "Pattern data",
        labels: [{ width: 40, label: "pattern" }],
        active: true,
      },
      {
        label: "Instruments",
        labels: [{ width: 30, label: "Instr" }],
        active: false,
      },
    ]);
