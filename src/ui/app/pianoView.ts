...
export default class AppPianoView extends AppPanelContainer {
    ...
    constructor() { // UI.app_pianoView
        ...
        this.octaveBox.setProperties({
            ...
            width: 150,
            font: UI.fontMed,
            onChange : (value) => {Input.setCurrentOctave(value)}
        });
    ...
    renderInternal(internal?: boolean) {
        ...
        if (this.needsRendering){
            ...
            while (keyX<this.width){
                ...
                const keyIndex = ((this.octave+thisOctave)*12) + this.keyMapWhite[octaveIndex] + 1;
            ...
            while (bkeyX<this.width){
                ...
                if (octaveIndex !== 2 && octaveIndex !== 6){
                    const keyIndex = ((this.octave+thisOctave)*12) + this.keyMapBlack[(octaveIndex*2)+1] + 1;
                    const bImg = this.keyDown[keyIndex] ? this.bKeyImgDown : this.bKeyImg;
                    this.ctx.drawImage(bImg,bkeyX,this.keyTop,bKeyWidth,this.bKeyHeight);
                    keyCounter++
                }
                counter++;
