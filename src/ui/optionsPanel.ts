...
export default class OptionsPanel extends Panel {
  ...
  constructor() {
    ...
    this.options.forEach((option) => {
      const labelBox = new Scale9Panel(
        0,
        0,
        20,
        20,
        Assets.panelDarkGreyScale9,
      );
      ...
      for (let i = 0; i < option.values.length; i++) {
        ...
        if (value) {
          ...
          if (button == null) {
            throw new Error(
              "Failed to generate image: buttonKey for OptionsPanel!",
            );
          }
  ...
  onResize() {
    ...
    const bWidth = Math.floor(
      (this.width - Layout.defaultMargin * (maxCols + 1)) / maxCols,
    );
    ...
    this.options.forEach((option, index) => {
      ...
      let thisLeft =
        Layout.defaultMargin + col * (bWidth + Layout.defaultMargin);
