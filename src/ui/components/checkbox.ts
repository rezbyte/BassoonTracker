...
export default class Checkbox extends Element {
	...
	constructor(x?: number, y?: number, w?: number, h?: number) { // Formerly UI.checkbox
		w = w || 14;
		h = h || 14;
		super(x,y,w,h);
	}
	...
	render(internal?: boolean){
		...
		if (this.needsRendering){
			...
			const stateImage = this.checked ? Y.getImage("checkbox_on") : Y.getImage("checkbox_off");
			if (stateImage == null) {
				console.error(`Cannot render checkbox without image: ${this.checked ? "checkbox_on" : "checkbox_off"}`);
				return;
			}
