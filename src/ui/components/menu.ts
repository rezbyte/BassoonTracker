...
export default class Menu extends Element {
    ...
    constructor(x: number, y: number, w: number, h: number, submenuParent: Element) { // UI.menu 
        super(x,y,w,h);
    ...
    setProperties(p: MenuProperties) {
		...
		if (p["background"]){
			this.background = new Scale9Panel(0,0,this.width,this.height,p["background"]);
    ...
    onClick(data: Touch) {
        ...
        if (selectedItem.subMenu){
            ...
            selectedItem.subMenu.setPosition((selectedItem.startX || 0) + xOffset,this.height);
    ...
    onHover(data: TouchData) {
        ...
        if ( selectedIndex<0) return;
        if (this.activeIndex !== undefined && this.activeIndex>=0 && this.activeIndex!==selectedIndex) {
    ...
    onKeyDown(keyCode: number, event: LegacyKeyboardEvent) {
        ...
        switch (keyCode){
            ...
            case 37:
                ...
                break;
            case 39:
                if (this.activeIndex !== undefined && this.items !== undefined && this.activeIndex>=0){
                    const subItem = this.getActiveSubItem();
                    if (subItem && subItem.item.subMenu && !subItem.item.subMenu.isVisible()){
                        subItem.menu.activateSubmenu(subItem.item);
                    }else{
                        this.activateSubmenu(Math.min(this.activeIndex+1,this.items.length-1));
                    }
                }
                handled = true;
                break;
    ...
    deActivate(clickedItem?: Submenu) {
        if (this.items != null && this.activeIndex != null && this.activeIndex>=0){
            const activeItem = this.items[this.activeIndex];
            if (activeItem && activeItem.subMenu){
    ...
    private getItemIndexAtPosition(x: number): number {
        if (this.items && this.items.length){
            for (let i = 0, max = this.items.length; i<max;i++){
                ...
                if (x>=startX-(this.itemMargin/2) && x<=startX+width+(this.itemMargin/2)){
                    return i;
                }
            }
        }
