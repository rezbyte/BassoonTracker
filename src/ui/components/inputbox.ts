...
interface InputBoxProperties extends Omit<ElementProperties, "zIndex">, Changeable<string>, ElementUndoProperties {
	onSubmit?: (value: string) => void,
	backgroundImage?: string,
	value?: string,
}

export default class InputBox extends Element implements Control<string>, Changeable<string> {
	private value = "";
	private prevValue = "";
	...
	constructor(initialProperties?: InputBoxProperties) { // formerly UI.inputbox
		super();
	...
	onKeyDown(keyCode: number, event: LegacyKeyboardEvent) {
		...
		switch(keyCode){
			case 8:// backspace
				if (this.value) {
					if (this.cursorPos>=0){
						this.setValue(this.value.substr(0,this.cursorPos) + this.value.substr(this.cursorPos+1));
						this.cursorPos--;
					}
				}
				handled = true;
			...
			case 46: // delete
				if (this.value) {
					if (this.cursorPos<this.value.length-1){
						this.setValue(this.value.substr(0,this.cursorPos+1) + this.value.substr(this.cursorPos+2));
					}
				}
				handled = true;
				break;
		...
		if (!handled && keyCode>31){
			...
			if (key.length === 1 && key.match(/[a-z0-9\._:\-\ #]/i)){
				this.setValue(this.value.substr(0,this.cursorPos+1) + key + this.value.substr(this.cursorPos+1));
				this.cursorPos++;
			}
			handled = true;
		}
