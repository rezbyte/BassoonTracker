...
import { ElementRangeProperties, ElementUndoProperties, Size, LegacyKeyboardEvent } from "../basetypes";
...
export interface NumberDisplayProperties extends ElementProperties, Changeable<number>, ElementUndoProperties, ElementRangeProperties {
    value?: number,
    padLength?: number,
    size?: Size,
    autoPadding?: boolean,
...
export default class NumberDisplay extends Element implements Control<number>, Changeable<number> {
    ...
    private static readonly fontOffsets: Record<Size, {x:number,y:number,c:number}> = {
        0:{x:4,y:3,c:0}, // small
    ...
    constructor(initialProperties?: NumberDisplayProperties) { // UI.numberDisplay 
        super();
   ...
   protected setPropertiesValues(p: NumberDisplayProperties) {
    ...
    render(internal?: boolean) {
        ...
        if (this.needsRendering){
            ...
            const backgroundImageName = this.hasFocus?"panel_inset_dark_active":"panel_inset_dark_inactive";
            const backgroundImage = Y.getImage(backgroundImageName);
            if (backgroundImage == null) {
                console.error(`Failed to get image: ${backgroundImageName} for NumberDisplay background!`);
                return;
            }
