...
export default class ListBox extends Element {
    ...
    constructor(x?: number, y?: number, w?: number, h?: number) { // UI.listbox
        ...
        this.scrollBar = new Scale9Panel((w ?? this.width)-28,18,16,(h ??this.height)-3,{
            img: Y.getImage("bar"),
            left:2,
            top:2,
            right:3,
        ...
        this.scrollBar.onDrag=(touchData) => {
            if (this.scrollBar.startDragIndex === undefined) {
                console.error("A ListBoxes scroll bar is missing startDragIndex for onHover!");
                return;
            }
            if (this.items.length>this.visibleIitems && this.scrollBarItemOffset){
                const delta =  touchData.deltaY;
                this.visibleIndex = Math.floor(this.scrollBar.startDragIndex + delta/this.scrollBarItemOffset);
    ...
    setSelectedIndex(index: number, internal?: boolean) {
		...
		this.setScrollBarPosition();
        this.refresh();
        if (!internal && this.onChange && this.previousSelectedIndex!=this.selectedIndex) this.onChange();
                ...
                if ((textY>0) && (textY<this.height)){
                    ...
                    if (targetCtx){
                        if (this.hoverIndex && this.hoverIndex+this.visibleIndex === i){
                            ...
                            targetCtx.fillRect(0,_y-highlightY,this.width-2,this.lineHeight);
                        }
                        ...
                        const icon = typeof item.icon === "string" ? Y.getImage(item.icon) : item.icon; 
                        if (icon){
                        ...
                        if (this.font){
                            ...
                            if (item.info){
                                ...
                                this.fontSmall.write(targetCtx,item.info,this.width-infoLength,_y,0);
                                const charWidth = this.font.getCharWidthAsFixed();
                                text = text.substr(0,Math.floor((this.width-infoLength-textX-26)/charWidth));
                            }
    ...
    private setScrollBarPosition(){
        ...
        if (this.centerSelection){this.visibleIitems = 1;}
        ...
        if (max>this.visibleIitems){
            ...
            this.scrollBarItemOffset = (startHeight - height) / (max-this.visibleIitems);
