...
export default class RadioGroup extends Element implements Changeable<number> {
	...
	constructor(x?: number, y?: number, w?: number, h?: number) { // Formerly UI.radioGroup
	...
	onClick(e: Touch){
		...
		this.setSelectedIndex(Math.floor((this.eventY-this.startY+this.buttonY)/this.itemHeight));
	};
	...
	setSelectedIndex(index: number, internal?: boolean) {
		...
		if (!internal && this.onChange && this.previousSelectedIndex!=this.selectedIndex) this.onChange(this.selectedIndex);
			...
			for (let i = 0, len = this.items.length;i<len;i++){
				...
				if (item.active){
					...
					if (buttonInactive == null) {
						console.error(`Missing image: ${buttonInactiveName} for RadioGroup!`)
						return;
					}
