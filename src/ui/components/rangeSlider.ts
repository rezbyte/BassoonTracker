...
export default class RangeSlider extends Element implements Changeable<number> {
	...
	constructor(initialProperties?: RangeSliderProperties) { // UI.rangeSlider
		super();
	...
	render(internal?: boolean) {
		if (this.needsRendering){
			...
			this.back.render();
			if (this.vertical){
                this.ctx.drawImage(this.knobVert,-1,this.knobTop,this.knobVert.width,this.knobVert.height);
			}else{
                this.ctx.drawImage(this.knob,this.knobLeft,-1,this.knob.width,this.knob.height);
			}
		}
	...
	onDrag(touchData: Drag) {
		if(this.vertical){
            ...
            if (this.maxHeight>this.knob.height){
            	const relMax = this.max - this.min;
            	const relValue = relMax - (Math.round(relMax * this.knobTop/this.maxHeight));
