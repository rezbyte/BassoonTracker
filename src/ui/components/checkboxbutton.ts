...
export class CheckboxButton extends Button {
	...
	constructor(properties?: CheckboxButtonProperties) { // Formerly UI.checkboxbutton
		super(0,0,20,20);
		...
		this.setProperties({
			...
			hoverBackground:properties?.hoverBackground || Assets.buttonDarkBlueActiveScale9,
			activeBackground:properties?.activeBackground || Assets.buttonDarkBlueActiveScale9,
			isActive:false,
	...
	renderInternal() {
		...
		if (stateImage == null) {
			console.error(`Failed to render CheckboxButton due to missing image: ${stateImageName}`)
			return;
		}
