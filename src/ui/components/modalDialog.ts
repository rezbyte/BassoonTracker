...
interface ModalDialogProperties extends Omit<ElementProperties, "type" | "zIndex"> {
    ok?: boolean,
    cancel?: boolean,
    input?: boolean
}
...
export class ModalDialog extends Element {
    ...
    constructor(initialProperties?: ModalDialogProperties) { // Formerly Ui.modalDialog
        super()
        this.background = new Scale9Panel(0,0,Math.floor(this.width/2),200, Assets.panelMainScale9);
        ...
        if (okButton == null) {
            throw new Error("Could not get the asset: buttonLight to initialize the OK button for a ModalDialog!");
        }
        ...
        if (cancelButton == null) {
            throw new Error("Could not get the asset: buttonLight to initialize the Cancel button for a ModalDialog!");
        }
    ...
    setProperties(p: ModalDialogProperties){
        ...
        this.background.setPosition(Math.floor((this.width-panelWidth)/2),Math.floor((this.height-panelHeight)/2));

        if (this.shouldRenderCancelButton){
            this.okButton.setPosition(this.background.left + Math.floor(this.background.width/2) - 110,this.background.top + this.background.height - 40);
            this.cancelButton.setPosition(this.background.left + Math.floor(this.background.width/2) + 10,this.background.top + this.background.height - 40);
        }else{
            this.okButton.setPosition(this.background.left + Math.floor(this.background.width/2) - 50,this.background.top + this.background.height - 40);
        }
            ...
            if (this.text){
                ...
                lines.forEach((line) => {
                    ...
                    if (UI.fontFT){
                        ...
                        textX = this.background.left + 10 + Math.floor((maxWidth - textLength)/2);
