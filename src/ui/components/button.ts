import { ElementLabelProperties, LabelStruct, TextAlignment } from "../basetypes";
...
// "active" -> isActive
export interface ButtonProperties extends ElementProperties, ElementLabelProperties {
...
export default class Button extends Element {
    ...
    constructor(x?: number, y?: number, w?: number, h?: number, text?: string) { // Formerly UI.button
        super(x,y,w,h);
        this.type = "button";
        this.isActive = false;
   ...
   setProperties(p: ButtonProperties) {
        ...
        if (this.activeBackground) this.activeBackground.setSize(this.width,this.height);
        if (this.hoverBackground) this.hoverBackground.setSize(this.width,this.height);
        ...
        if (this.needsRendering){
            ...
            if (this.backgroundImage){
                ...
                if (this.isActive && this.activeBackground){
                    ...
                    if (this.activeImage){
                        const imgY = Math.floor((this.height-this.activeImage.height)/2);
            ...
            if (this.label && drawFonts){
                ...
                const textY = Math.floor((this.height-fontSize)/2) + (this.isActive?this.paddingTopActive:this.paddingTop);
                let textX = this.paddingLeft;
                if (this.font){
                    if (this.textAlign === TextAlignment.center){
                        let textLength = this.label.length * fontWidth;
                        if (!this.font.fixedWidth) textLength = this.font.getTextWidth(this.label,0);
