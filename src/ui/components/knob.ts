...
export default class Knob extends Element implements Changeable<number> {
	...
	private readonly properties = ["left","top","width","height","name","font","label","textAlign","paddingTop","disabled"];
	...
	constructor(initialProperties?: KnobProperties) { // Formerly UI.knob
		super()
		this.type = "knob";
	...
	render(internal?: boolean){
		if (this.needsRendering){
			...
			this.ctx.drawImage(this.isDisabled ? this.imgDisabled : this.img,-w,-h,imgw,imgh);
			...
			this.ctx.fillStyle = this.isDisabled ? "rgba(170,170,170,0.5)" : "rgba(130,200,255,0.5)";
			this.ctx.beginPath();
			...
			if (this.label){
				...
				if (UI.fontSmall) {
					UI.fontSmall.write(this.ctx,this.label,labelX,imgh + this.padding + 4);
				} else {
					console.error("Could not render label for Knob due to missing small font.");
				}
