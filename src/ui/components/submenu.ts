...
export default class Submenu extends Element {
    ...
    constructor(x?: number, y?: number, w?: number, h?: number) { // UI.submenu
        super(x,y,w,h);
        this.type = "submenu";
    }
    ...
    onHover(data: TouchData) {
        if (this.eventY === undefined) {
            console.error("Submenu expected eventY to be processed for onHover but it was not!");
            return
        }
    ...
    activateSubmenu(item: MenuItem) {
        if (UI.mainPanel == null) {
            console.error("Cannot activate Submenu due to the main panel not being initialized!");
            return;
        }
        if (!item.subMenu){
            if (this.parent == null) {
                console.error("Cannot activate Submenu due to there being no parent to attach a new Submenu to!");
                return;
            }
        ...
        if (this.needsRendering){
            ...
            for (let i = 0; i<=max;i++){
                ...
                if (item.label){
                    ...
                    UI.fontFT.write(this.ctx,label,textX + this.paddingLeft,textY + this.paddingTop);
                }
                ...
                if (item.subItems){
                    ...
                    UI.fontMed.write(this.ctx,">",this.width-16,textY + this.paddingTop + 2);
                }
