...
export default class DiskOperationTargets extends Panel {
	...
	constructor() { // UI.DiskOperationTargets
		super();
		...
		this.selectionTarget.onChange = (selectedIndex) => {
			EventBus.trigger(EVENT.diskOperationTargetChange, this.selectionTarget.getSelectedItem());
		};
		...
		EventBus.on(EVENT.diskOperationActionChange,(target: RadioGroupItem | undefined) => {
	...
	private removeTarget(list: Target[], target: string) {
	    const index = list.findIndex((item) => {return item.target === target;})
        if (index>=0){
            list.splice(index, 1)
        }
	}
