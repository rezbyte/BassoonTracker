var Editor = (function () {
  ...
  me.putNote = function (instrument, period, noteIndex, volume) {
    var note =
      Tracker.getSong().patterns[currentPattern][currentPatternPos][
        currentTrack
      ] || new Note();
    var editAction = StateManager.createNoteUndo(
      currentPattern,
      currentTrack,
      currentPatternPos,
      note
  ...
  me.copyTrack = function (trackNumber) {
    ...
    for (var i = 0; i < length; i++) {
      var note =
        Tracker.getSong().patterns[currentPattern][i][trackNumber] ||
        new Note();
  ...
  me.renderTrackToBuffer = function (fileName, target) {
    ...
    Audio.stopRendering(function (renderedBuffer) {
      ...
      if (saveToFile) {
        ...
        fileName =
          fileName ||
          Tracker.getSong().title.replace(/ /g, "-").replace(/\W/g, "") +
            ".wav" ||
          "module-export.wav";
    ...
    function checkSample() {
      if (Tracker.getTrackerMode() === TRACKERMODE.PROTRACKER) {
        ...
        if (instrument.sample.length > 131070) {
          ...
          dialog.setText(
            "Warning//The maximum sample lenght in .MOD format is 128kb//If you save in .MOD format/this sample will be truncated.//Please try downsampling or trimming the sample/to below 131072 bytes/or switch to .XM format"
          );
