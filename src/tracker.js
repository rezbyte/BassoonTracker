...
var Tracker = (function () {
          ...
          if (stepResult.patternDelay) {
            ...
            if (p >= thisPatternLength || stepResult.patternBreak) {
              ...
              if (Tracker.getPlayType() == PLAYTYPE.song) {
                var nextPosition = stepResult.positionBreak
          ...
          if (trackNote && trackNote.time && trackNote.scheduled) {
            ...
            if (trackNote.scheduled.volume) {
              if (time + delay >= trackNote.scheduled.volume) {
                var scheduledtime = instrument.scheduleEnvelopeLoop(
            ...
            if (trackNote.scheduled.panning) {
              if (time + delay >= trackNote.scheduled.panning) {
                scheduledtime = instrument.scheduleEnvelopeLoop(
  ...
  function playNote(note, track, time, songPos) {
    ...
    if (notePeriod && !instrumentIndex) {
      ...
      defaultVolume =
        typeof trackNotes[track].currentVolume === "number"
          ? trackNotes[track].currentVolume
          : defaultVolume;

      if (
        SETTINGS.emulateProtracker1OffsetBug &&
        instrumentIndex &&
        trackEffectCache[track].offset
      ) {
    ...
    if (noteIndex && me.inFTMode()) {
      ...
      if (noteIndex === NOTEOFF) {
        var offInstrument =
          instrument || me.getInstrument(trackNotes[track].currentInstrument);
        if (offInstrument) {
    ...
    switch (note.effect) {
      ...
      case 9:
        ...
        if (!value && trackEffectCache[track].offset) {
          value =
            trackEffectCache[track].offset.stepValue ||
            trackEffectCache[track].offset.value ||
            0;
        }
        var stepValue = value;
    ...
    if (doPlayNote && instrumentIndex && notePeriod) {
      ...
      if (instrument) {
        trackNotes[track] = instrument.play(
          noteIndex,
          notePeriod,
          volume,
          track,
      ...
      trackEffectCache[track].defaultSlideTarget =
        trackNotes[track].startPeriod;
    }
  ...
  function cutNote(track, time) {
    ...
    try {
      if (trackNotes[track].source) {
        ...
        gain.setValueAtTime(
          trackNotes[track].currentVolume / 100,
          time - 0.002
        );
  ...
  function applyAutoVibrato(trackNote, currentPeriod) {
    ...
    if (instrument) {
      ...
      var targetPeriod = instrumentVibratoFunction(
        currentPeriod,
  ...
  function applyEffects(track, time) {
      ...
      if (trackNote.source) {
        ...
        for (var tick = 1; tick < steps; tick++) {
          ...
          if (
            ...
            trackEffectCache[track].glissando
          ) {
    ...
    if (effects.cutNote) {
      if (trackNote.volume) {
        trackNote.volume.gain.setValueAtTime(
          0,
          time + effects.cutNote.value * tickTime
        );
      }
    ...
    if (effects.noteOff) {
      ...
      if (instrument) {
        trackNote.currentVolume = instrument.noteOff(
          time + effects.noteOff.value * tickTime,
          trackNote
        );
      }
    }
    ...
    if (effects.reTrigger) {
      ...
      while (triggerCount < ticksPerStep) {
        ...
        trackNotes[track] = Audio.playSample(
          instrumentIndex,
          notePeriod,
          volume,
          track,
  ...
  me.setPatternLength = function (value) {
    ...
    if (currentLength < patternLength) {
      ...
      song.patterns[currentPattern] = song.patterns[currentPattern].splice(
        0,
        patternLength
      );
  ...
  me.getFileName = function () {
    return (
      song.filename ||
      (song.title
        ? song.title.replace(/ /g, "-").replace(/\W/g, "") + ".mod"
        : "new.mod")
    );
  };
