import {EVENT, STEREOSEPARATION,  NotePeriod, NOTEPERIOD, NOTEOFF, PC_FREQUENCY_HALF, AMIGA_PALFREQUENCY_HALF, TRACKERMODE} from "./enum";
import Tracker, { periodNoteTable, FTNotes, noteNames, Effects, periodFinetuneTable, nameNoteTable } from "./tracker";
...
class Audio {
    ...
    createAudioConnections(audioContext: BaseAudioContext, destination?: AudioNode) {
    ...
    init(audioContext?: BaseAudioContext, destination?: AudioNode) {
        ...
        if (!this.isRendering){
			...
			EventBus.on(EVENT.trackCountChange,(trackCount: number) => {
				for (let i = this.filterChains.length; i<trackCount;i++)this.addFilterChain();
    ...
    checkState() {
        if (this.currentContext){
            if (this.currentContext.state === "suspended" && Audio.isAudioContext(this.currentContext)){
    ...
    private static isAudioContext(context: BaseAudioContext): context is AudioContext {
        return (context as AudioContext).resume != null && (context as AudioContext).createMediaStreamDestination != null
    }

    playSample(index: number, period: number, volume: number | undefined, track: number | undefined, effects: Effects | undefined, time: number | undefined, noteIndex: number): NoteInfo | null {

        let audioContext: BaseAudioContext;
        if (this.isRendering){
            if (this.offlineContext == null) {
                console.error("Offline context is missing when playing a sample while rendering! Has rendering been started?");
                return null;
            }
        ...
        if (typeof track === "undefined") track = (this.hasUI ? Editor.getCurrentTrack() : 0);
    ...
    playSilence() {
        ...
        if (this.currentContext){
            if (this.masterVolume == null) {
                console.error("Need the master volume gain node to be initialized to play silence!")
                return;
            }
	...
	playRaw(data: {length: number}, sampleRate: number) {
        if (this.masterVolume == null) {
            console.error("Need the master volume gain node to be initialized to play a raw sample!")
            return;
        }
		if (this.currentContext && data && data.length){
			const sampleBuffer = this.currentContext.createBuffer(1,data.length,this.currentContext.sampleRate);
    ...
    startRecording() {
        if (this.masterVolume == null) {
            console.error("Need the master volume gain node to be initialized to start recording!")
            return;
        }
        if (!this.isRecording){
    ...
    startRendering(length: number){
        if (!Audio.isAudioContext(this.currentContext)) {
            console.error("Main context is already the offline context while attempting to start rendering!");
            return;
        }
    ...
    stopRendering(next: (renderedBuffer: AudioBuffer) => void) {
        ...
        if (this.offlineContext == null) {
            console.error("Offline context is missing when stoping rendering! Has rendering been started?");
            return;
        }
        if (this.onlineContext == null) {
            console.error("Online context is missing when stoping rendering! Has rendering been started?");
            return;
        }
     ...
     */
    semiTonesFrom(source: AudioBufferSourceNode, root: number, semitones: number, finetune: number): AudioBufferSourceNode {
            ...
            if (rootNote){
                ...
                if (targetName){
                    ...
                    if (targetNote){
                        ...
                        if (finetune) {result = this.getFineTuneForPeriod(result,finetune)}
                    }
                }
            }else{
        ...
        if (Tracker.inFTMode()){
            ...
            if (targetIndex && instrument && instrument.getFineTune()){
                if (Tracker.useLinearFrequency){
                    ...
                    result = this.getFineTuneForNote(targetIndex,instrument.getFineTune());
                }
            }
    ...
    setMasterVolume(value: number, time?: number) {
        if (this.masterVolume == null) {
            console.error("Cannot set the master volume without the master volume gain being initialized!");
            return;
        }
        time=time||this.currentContext.currentTime;
        value = value*0.7;
	...
	slideMasterVolume(value: number, time?: number) {
        if (this.masterVolume == null) {
            console.error("Cannot slide the master volume without the master volume gain being initialized!");
            return;
        }
		time=time||this.currentContext.currentTime;
		value = value*0.7;
