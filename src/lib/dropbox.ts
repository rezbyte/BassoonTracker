...
class DropboxService {
  ...
  call(
    endpoint: keyof typeof DropboxService.endpointMapping | string,
    apiArgs?: { responseType?: XMLHttpRequestResponseType; path: string },
    content?: Blob,
    handlers?: CallHandlers | OnCompleteHandler,
  ): Promise<CallReturn> | null {
    ...
    const config: {
      ...
      DropboxService.endpointMapping[
        endpoint as keyof typeof DropboxService.endpointMapping
      ] || {};
    const baseUri = config?.baseUri || DropboxService.api;
    const format = config?.format || "rpc";
    const contentType =
      config.contentType || config.contentType === null
        ? null
        : DropboxService.contentTypeMapping[format];

    //var lastArg = args[args.length - 1];
    //let handlers = (args.length > 2 && (DropboxService.isObject(lastArg) || DropboxService.isFunction(lastArg))) ? lastArg : {};
    if (DropboxService.isFunction(handlers))
    ...
    r.setRequestHeader(
      "Authorization",
      "Bearer " +
        (this.tokenStore("__dbat") ||
          "000000000000000000000000_00000-000000000000000000000000000000000"),
    );
    ...
    if (handlers?.onError || this.globalErrorHandler)
    ...
    let requestPayload: string | Blob | null =
      content && format == "content-upload" ? content : null; // (args.length > 2 && format == 'content-upload') ? args[2] : undefined;
    requestPayload =
      requestPayload ||
      (apiArgs && format == "rpc" ? JSON.stringify(apiArgs) : null);
    if (requestPayload) {
  ...
  authenticate(
    apiArgs: { client_id: string; redirect_uri?: string },
    handlers:
      | {
          onComplete: () => void;
          onError?: (params: { state: string }) => void;
        }
      | (() => void),
  ) {
    handlers = handlers || {};
    ...
    if (params.state && csrfToken && params.state == csrfToken) {
      ...
      window.location.assign(
        "https://www.dropbox.com/1/oauth2/authorize?response_type=token&" +
          "client_id=" +
          encodeURIComponent(apiArgs.client_id) +
          "&" +
          "redirect_uri=" +
          encodeURIComponent(apiArgs.redirect_uri) +
          "&" +
          "state=" +
  ...
  private static paramsFromUrlHash(): UrlParameters {
    return window.location.hash
      .replace(/^#/, "")
      .split("&")
      .reduce(function (o: Partial<UrlParameters>, entry: string) {
        if (entry == "") return o;
        const splitEntry = entry.split("=");
        o[decodeURIComponent(splitEntry[0]) as keyof UrlParameters] =
          decodeURIComponent(splitEntry[1]);
        return o;
      }, {}) as UrlParameters;
  }
}
