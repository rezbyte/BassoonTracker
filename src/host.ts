...
class Host {
	...
	init() {
	    if (typeof HostBridge === "object"){
			...
			if (typeof this.hostBridge.useUrlParams === "boolean") this.useUrlParams = this.hostBridge.useUrlParams;
			if (typeof this.hostBridge.useDropbox === "boolean") this.useDropbox = this.hostBridge.useDropbox;
			if (typeof this.hostBridge.showInternalMenu === "boolean") this.showInternalMenu = this.hostBridge.showInternalMenu;
			if (typeof this.hostBridge.useWebWorkers === "boolean") this.useWebWorkers = this.hostBridge.useWebWorkers;
	    }
	};
	...
	getVersionNumber(): string {
		...
		if (this.hostBridge && this.hostBridge.getVersionNumber) 	return this.hostBridge.getVersionNumber();
		return "dev";
	};
	...
	getBuildNumber(): number {
		...
		if (this.hostBridge && this.hostBridge.getBuildNumber) return this.hostBridge.getBuildNumber();
		return new Date().getTime();
	};

	signalReady() {
		if (this.hostBridge && this.hostBridge.signalReady) this.hostBridge.signalReady();
	};
