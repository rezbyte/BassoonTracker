export default Nibbles = (function () {
  var me = {
    name: "nibbles",
  };
  ...
  var Game = function () {
    ...
    game.checkHighScore = function (score) {
      var index = highScoreTable.findIndex(function (item) {
        return item.score < score;
      });
      if (index > 0) {
        host.UI.showDialog(
          "GAME OVER//But don't be sad/because you made the enternal/HIGHSCORE HALL OF FAME//Enter your name",
          function (value) {
            setTimeout(function () {
              value = value || "John Doe";
              highScoreTable.splice(index, 0, {
                name: value,
                score: score,
                level: level,
              });
    ...
    game.setLevel = function () {
      ...
      if (marginLeft < 0 || marginTop < 0)
        console.warn("Warning: level doesn't fit on playing area");
      ...
      if (!highScore) {
        ...
        highScore.render = function () {
          ...
          if (highScore.needsRendering) {
            ...
            titleFont.write(
              highScore.ctx,
              "Nibbles Highscore List of Fame",
              marginLeft,
              marginTop + 3,
              1
            );
            ...
            highScoreTable.forEach(function (item, i) {
              if (i < displayCount) {
                ...
                highScore.ctx.drawImage(
                  getLevelSprite(item.level),
                  x + colW - 60,
                  y + 1
                );
              }
  ...
  var GameControls = function () {
    ...
    var infoPanel = host.UI.scale9Panel(
      0,
      0,
      20,
      20,
      host.UI.Assets.panelMainScale9
    );
    infoPanel.ignoreEvents = true;

    var infoLabel = host.UI.label({
      textAlign: "center",
      label: "Ready Player 1?",
      font: fontFT,
    });
    ...
    var labelBoxOptions = host.UI.scale9Panel(
      0,
      0,
      20,
      20,
      host.UI.Assets.panelDarkGreyScale9
    );
    ...
    var labelBoxDifficulty = host.UI.scale9Panel(
      0,
      0,
      20,
      20,
    ...
    var insetScale9 = {
      ...
      right: 5,
      bottom: 5,
      img: host.Y.getImage("panel_inset_dark_inactive"),
    };
    ...
    gameControls.resize = function () {
      if (host.Layout.prefered === "col3") {
        ...
        backgroundPanel.setSize(
          host.Layout.col32W + 4,
          renderTarget.height + 4
        );
        ...
        radioGroup.setProperties({ size: "small" });
      ...
      highScoreButton.setLabel(
        highScoreButton.width < 100 ? "High" : "HighScore"
      );
    };
    ...
    gameControls.setScore = function (score) {
      ...
      var s = "" + score;
      var len = Math.floor(
        scoreNumber.width / (scoreNumber.getFont().charWidth + 2)
      );
      while (s.length < len) {
        s = " " + s;
      }
    ...
    host.EventBus.trigger(host.EVENT.pluginRenderHook, {
      ...
      setRenderTarget: function (element) {
        ...
        renderTarget.onHide = function () {
          ...
          if (registeredRenderEvent)
            host.EventBus.off(host.EVENT.screenRender, registeredRenderEvent);
        ...
        registeredRenderEvent = host.EventBus.on(
          host.EVENT.screenRender,
          function () {
