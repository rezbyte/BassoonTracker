module.exports = function (grunt) {
  grunt.initConfig({
    ...
    uglify: {
      options: {
        banner:
          ...
          'build <%= grunt.template.today("yyyy-mm-dd") %> - Full source on <%= pkg.repository %> */',
        mangle: {},
        compress: {
      ...
      friend: {
        files: {
          "hosts/FriendOs/build/bassoontracker-min.js": [
            "hosts/FriendOs/build/bassoontracker.js",
          ],
        },
      },
      player: {
    ...
    copy: {
      friend: {
        files: [
          {
            src: ["hosts/FriendOs/BassoonTracker.jsx"],
            dest: "hosts/FriendOs/build/",
            flatten: true,
            expand: true,
          },
          {
            src: ["skin/spritemap_v2.json", "skin/spritesheet_v2.png"],
            dest: "hosts/FriendOs/build/skin/",
            flatten: true,
            expand: true,
          },
          {
            ...
            dest: "hosts/FriendOs/build/data/",
            flatten: true,
            expand: true,
          },
          {
            src: ["demomods/Tinytune.mod"],
            dest: "hosts/FriendOs/build/demomods/",
            flatten: true,
            expand: true,
          },
          {
            src: ["skin/icon_256.png"],
            dest: "hosts/FriendOs/build/data/icon.png",
          },
          {
            src: ["skin/screenshot3.png"],
            dest: "hosts/FriendOs/build/data/preview.png",
          },
        ],
      },
      regpack: {
        files: [{ src: ["script/b"], dest: "player/b-pack.js" }],
      },
    },
    clean: {
        ...
        replacements: [
          {
            ...
            to: function (matchedWord) {
              return grunt.template.process(
                '<%= pkg.version %>-build<%= grunt.template.today("yyyymmdd.hhMM") %>'
              );
            },
          },
          {
    ...
    sprite: {
      all: {
        src: ["skin/src/*.png", "skin/src/icons_small/*.png"],
    ...
    regpack: {
      pack: {
        ...
        files: [{ src: ["player/b-min.js"], dest: "script/b" }],
      },
    },
    compress: {
      main: {
        ...
        files: [{ expand: true, src: ["b"], cwd: "script/", dest: "/" }],
      },
    },
    base64: {
      target: {
  ...
  grunt.registerTask("wrap", [
    "replace:buildnumber",
    "replace:versioncheck",
    "concat:tracker",
  ]);
  grunt.registerTask("tracker", [
    "replace:buildnumber",
    "replace:versioncheck",
    "concat:tracker",
    "uglify:tracker",
  ...
  grunt.registerTask("miniplayer", [
    "concat:player",
    "run",
    "uglify:playerSqueezed",
  ]);
  grunt.registerTask("default", ["tracker"]);
  grunt.registerTask("sprites", ["sprite"]);
  grunt.registerTask("friend", [
    "clean:friend",
    "concat:friend",
    "uglify:friend",
    "copy:friend",
    "replace:friend",
    "replace:friendpackage",
    "clean:friendjs",
  ]);
